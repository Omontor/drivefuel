...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.trivia.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.trivia.update", [$trivia->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('questionarie') ? 'is-invalid' : '' }}" name="questionarie_id" id="questionarie_id" required>
                    @foreach($questionaries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('questionarie_id') ? old('questionarie_id') : $trivia->questionarie->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('content') ? 'is-invalid' : '' }}" type="text" name="content" id="content" value="{{ old('content', $trivia->content) }}" required>
