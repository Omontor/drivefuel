...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.questionaries.update", [$questionarie->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.questionarie.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $questionarie->name) }}" required>
                @if($errors->has('name'))
