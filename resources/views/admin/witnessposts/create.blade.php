...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.witnesspost.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.witnessposts.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('witness') ? 'is-invalid' : '' }}" name="witness_id" id="witness_id" required>
                    @foreach($witnesses as $id => $entry)
                        <option value="{{ $id }}" {{ old('witness_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="user_id">{{ trans('cruds.witnesspost.fields.user') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.witnesspost.fields.user_helper') }}</span>
            </div>
            <div class="form-group">
